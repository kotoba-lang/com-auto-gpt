(ns auto_gpt.main
  "Kotodama WASM entrypoint for the Auto_gpt clean-room actor (L4) — Clojure port.
  ...
  facts (`auto_gpt.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "auto_gpt")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"auto_gpt.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "auto_gpt-compat" :tier tier :entities entities} 200])
...
(defn start! [] :auto_gpt-compat/ready)
